...
function createOverlay() {
    ...
    urgentButton.textContent = 'Sağlığımı Önemsemiyorum';
    urgentButton.disabled = true;
    urgentButton.classList.add('disabled');


    urgentButton.onclick = () => {
        chrome.runtime.sendMessage({ action: "closeOverlay" });
        const remainingSeconds = parseInt(timer.textContent, 10);

        chrome.storage.sync.get({ remainingSeconds: 0 }, (items) => {
            const totalRemainingSeconds = items.remainingSeconds + remainingSeconds;
    
            chrome.storage.sync.set({ remainingSeconds: totalRemainingSeconds }, () => {
                console.log(`Remaining seconds (${remainingSeconds}) added. Total: ${totalRemainingSeconds}`);
            });
        });
    ...
    setTimeout(() => {
        ...
        urgentButton.classList.remove('disabled');
